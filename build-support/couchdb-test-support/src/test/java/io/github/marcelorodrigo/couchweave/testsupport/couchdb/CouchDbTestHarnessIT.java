...
class CouchDbTestHarnessIT {
    ...
    void shouldProvideAnAuthenticatedReadyCouchDbDatabaseOnADynamicPort(CouchDbTestDatabase database)
        throws Exception {
        // given
        var authorization = Base64.getEncoder().encodeToString(
            (database.username() + ":" + database.password()).getBytes(StandardCharsets.UTF_8)
        );
