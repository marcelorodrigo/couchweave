...
| Repositories | Synchronous document CRUD | Supported | `CouchWeaveRepository` extends Spring Data `CrudRepository`; IDs must be `String`; CRUD delegates to mapped operations while preserving revision semantics. `saveAll` and collection operations are sequential and non-atomic. `findAll` and `count` filter by the exact discriminator, with no pagination or sorting. Custom repository fragments compose through standard Spring Data `RepositoryFragment` infrastructure and are verified alongside the proxied CRUD methods. |
| Repositories | Enabling/configuration | Supported | `@EnableCouchWeaveRepositories` activates repositories in a plain Spring context without Spring Boot. It scans standard Spring Data attributes and supplies default infrastructure (REST client builder, mapping context, custom conversions, converter, and operations); any of those beans can be overridden. Missing `CouchDbClientSettings` fails context startup, while custom operations and mapping context allow startup without settings. Startup performs no network access. |
